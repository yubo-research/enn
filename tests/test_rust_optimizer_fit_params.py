...
def test_rust_optimizer_passes_hnsw_hannoy_index_driver():
    config = turbo_enn_config(
        enn=ENNSurrogateConfig(index_driver=ENNIndexDriver.HNSW_HANNOY)
    )
    overrides = _config_to_rust_overrides(config)

    assert overrides is not None
    assert overrides["index_driver"] == "hnsw_hannoy"
