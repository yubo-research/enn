...
def subsample_loglik(
    ...
    *,
    ...
    from .._rust import subsample_loglik as _rust_subsample_loglik
    ...
    # Extract Rust model (the Python model wraps the Rust model)
    if not isinstance(model, PyENN):
        raise TypeError(f"Expected EpistemicNearestNeighbors, got {type(model)}")

    if model._rust_model is None:
        raise RuntimeError(
            "Rust model is not available. Ensure backend='rust' was used."
        )

    # Convert Python RNG to seed for Rust RNG
    seed = int(rng.integers(0, 2**63 - 1))

    # Convert Python ENNParams to Rust ENNParams
...
def enn_fit(
    ...
    *,
    ...
    from .._rust import enn_fit as _rust_enn_fit
    from .._rust import ENNParams as RustENNParams
    from .enn_class import EpistemicNearestNeighbors as PyENN
